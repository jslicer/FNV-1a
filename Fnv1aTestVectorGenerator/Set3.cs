...
internal sealed class Set3(TextWriter? writer = null) : SetBase(writer)
{
    ...
    public override async Task PerformAsync(CancellationToken token = default)
    {
        ...
        await WriteLineAsync(
            await "chongo was he".Test0Async(token).ConfigureAwait(false),
            token).ConfigureAwait(true);
        await WriteLineAsync(
            await "chongo was her".Test0Async(token).ConfigureAwait(false),
            token).ConfigureAwait(true);
        await WriteLineAsync(
            await "chongo was here".Test0Async(token).ConfigureAwait(false),
            token).ConfigureAwait(true);
        await WriteLineAsync(
            await "chongo was here!".Test0Async(token).ConfigureAwait(false),
            token).ConfigureAwait(true);
        await WriteLineAsync(
            await "chongo was here!\n".Test0Async(token).ConfigureAwait(false),
            token).ConfigureAwait(true);
        //// ReSharper enable StringLiteralTypo
    }
}